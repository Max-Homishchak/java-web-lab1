...
import com.example.spacecatsmarket.web.exception.BadRequestResponse;
...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatusCode status,
        ...
        return ResponseEntity.status(BAD_REQUEST).body(createBadRequestResponse((ServletWebRequest) request, createBadRequestDetailMessage(validationResponse, ex.getObjectName()), validationResponse));
    }

    private BadRequestResponse createBadRequestResponse(ServletWebRequest request, String message, List<ParamsViolationDetails> errorFields) {
       return BadRequestResponse.builder()
                .error("Bad Request")
                .invalidParams(errorFields)
                .status(400)
    ...
    private String createBadRequestDetailMessage(List<ParamsViolationDetails> validationResponse, String objectName) {
        StringBuilder detailMessage = new StringBuilder("Validation failed for object '").append(objectName).append("': ");
        validationResponse.forEach(err -> detailMessage.append(err.getReason()).append(", "));
        return detailMessage.substring(0, detailMessage.length() - 2);
    }
